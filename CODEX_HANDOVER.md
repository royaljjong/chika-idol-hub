...
| G 확장 가지 | 준비 중 | 검색 경로는 공개, 그라비아·공지는 빈 상태와 격리 후보만 운영; 콘텐츠 승격은 별도 정책 게이트 |
...
- 지도 핀과 동등한 키보드 공연장 버튼 목록을 항상 제공하며 선택 공연장의 Google Maps·일정 링크로 연결한다. 지도 실패·좌표 없음에도 일정 목록을 유지한다.
- 그룹 상세 profile/live 탭은 방향키·Home·End와 roving 포커스를 지원하고, `?tab=live` 직접 진입은 첫 서버 HTML부터 live 패널을 렌더한다. 공개 공연과 후보는 분리한다.
...
- 일일 보고는 그라비아 후보 수·검토 상태·안전 검토 대기도 함께 집계하며 후보를 공개 데이터로 쓰지 않는다.
...
- 2026-09-01 수동 갱신기 재검증은 후보 55건으로 통과했다. 이후 2026-09-02 실제 일일 갱신에서 후보 3건이 추가되어 현재 후보는 58건이며, 최신 보고서 `success=true`; 향후 생일 19명·미확인 생일 28명이다.
...
5. 공연 후보 58건 중 공개 연결 45건, 제외 4건(사진집/미디어 2·생일 2)이다. 2026-09-02 추가된 FRUITS ZIPPER 3건은 검토 대기로 보존한다. ASOBIEXPO HAWAII 1건은 공식 시간·일반권 가격 발표 대기이며, タイトル未定의 상세 부족 3건·분류 대기 2건은 공식 원문을 계속 조사한다.
6. MVP G의 검색 경로와 그라비아·공지 빈 상태 UI는 공개돼 있다. 실제 그라비아·공지 콘텐츠 승격은 별도 사용자 승인 대기이며, 승인 전 후보 자동 승격·외부 전송을 수행하지 않는다.
