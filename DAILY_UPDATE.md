...
이 명령은 공식 일정 공급원에서 공연 후보를 갱신하고, 기존 공식 생일을 기준으로 오늘부터 30일 이내 생일과 생일 미확인 멤버를 보고한다. 또한 분리된 이미지 후보 큐와 그라비아 후보 큐의 검토·권리·안전 상태를 읽기 전용으로 요약한다. 네트워크 결과는 `data/chika-live-candidates.json`에만 저장하며 공개 공연·멤버·생일·이미지·그라비아를 자동 게시하지 않는다.
...
그라비아 후보는 `data/chika-gravure-candidates.json`의 검토 상태와 `safetyReviewStatus`를 보고한다. 일일 실행은 후보 수집·승격이나 미성년자 안전 판정을 자동 수행하지 않는다.
